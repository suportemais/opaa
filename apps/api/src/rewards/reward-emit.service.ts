...
import { generateRewardCode, buildRewardDeepLink } from '../domain/rewards/code';
import { resolveCustomerKey, type CustomerKeyInput } from '../domain/rewards/customer-key';
...
export type RewardEmitResult =
  ...
  | { status: 'already_issued'; couponId: string; code: string; customerKey: string }
...
export class RewardEmitService {
  ...
  async emitForCompletedResponse(input: EmitForCompletedResponseInput): Promise<RewardEmitResult> {
    ...
    const appBaseUrl = (this.config.get<string>('MM_APP_BASE_URL') ?? '').trim();

    let created:
      | { id: string; code: string; amountCents: number | null; mmCompanyId: string | null; expiresAt: Date | null }
      | null = null;
    ...
    if (!created) {
      this.logger.error(`Failed to allocate unique reward code for campaign ${campaign.id}`);
  ...
  private async findEligibleCampaign(tenantId: string, surveyId: string): Promise<EligibleCampaign | null> {
    const now = new Date();
    ...
    if (typeof row.rewardAmountCents !== 'number' || row.rewardAmountCents <= 0) return null;
  ...
  private resolveExpiresAt(campaign: EligibleCampaign): Date | null {
    if (typeof campaign.validityDays === 'number' && campaign.validityDays > 0) {
